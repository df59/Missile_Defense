...
export default class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    const title = this.add.text(this.cameras.main.width / 2, this.cameras.main.height / 2 - 250, 'Game Over', { fontSize: '32px', fill: '#fff' }).setOrigin(0.5);

    // Display the score
    const scoreText = this.add.text(this.cameras.main.width / 2, this.cameras.main.height / 2 - 150, `Your Score: ${this.score}`, { fontSize: '24px', fill: '#fff' }).setOrigin(0.5);
    ...
    const submitButton = this.createButton(this.cameras.main.width / 2, this.cameras.main.height / 2 + 100, 'Submit Score', () => {
      let playerName = this.nameInput.getChildByName('name').value;
      console.log(`Player Name: ${playerName}, Score: ${this.score}`);
      // TODO "submit score" logic
    });
