...
export default class TurretUpgradeScene extends Phaser.Scene {
  ...
  createButton(x, y, text, upgradeKey, callback) {
    ...
    if (this.turret.upgrades[upgradeKey]) {
      ...
      costText = this.add
        .text(x + 180, y, `${this.turret.upgrades[upgradeKey].cost}`, {
          fontSize: "24px",
          fill: "#fff",
        })
        .setOrigin(0.5);
      tierText = this.add
        .text(x + 360, y, `${this.turret.upgrades[upgradeKey].tier}`, {
          fontSize: "24px",
          fill: "#fff",
        })
        .setOrigin(0.5);
    ...
    button.on("pointerover", () => {
      if (
        this.turret.upgrades[upgradeKey] &&
        this.playScene.funds < this.turret.upgrades[upgradeKey].cost
      ) {
      ...
      if (this.turret.upgrades[upgradeKey]) {
        this.descriptionText.setText(
          this.turret.upgrades[upgradeKey].description
        );
      }
    });
